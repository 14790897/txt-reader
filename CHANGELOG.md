...
- 新增「配色设置 (Color Setup)」命令：命令面板/右键菜单内直接管理对话样式与自定义调色板（添加/删除/清空颜色），改完立即预览
- 修复推理模型兼容：显式 max_tokens 防止思考耗尽额度，content 为空时回退 reasoning_content（deepseek-flash 实测通过）
- 修复 off 模式下配置变更误清手动配色的问题（手动命令的配色结果保留）
- E2E 15 条全绿（新增命令内配色管理用例）
